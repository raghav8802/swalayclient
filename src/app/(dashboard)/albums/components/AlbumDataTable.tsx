...
export const albumColumns: ColumnDef<Album>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const encodedId = btoa(album._id);
      return (
        <Link
          className="ms-2 text-blue-600"
          href={`/albums/viewalbum/${encodedId}`}
        >
          {album.title}
        </Link>
      );
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div>
        <i className="bi bi-person mr-2"></i> {row.getValue("artist")}
      </div>
    ),
  },
  {
  ...
  },
  {
    ...
    header: "Release Date",
    cell: ({ row }) => (
      <div>{new Date(row.getValue("releasedate")).toLocaleDateString()}</div>
    ),
  },
];
