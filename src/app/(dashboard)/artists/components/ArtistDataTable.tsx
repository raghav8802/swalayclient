...
export const artistColumns: ColumnDef<Artist>[] = [
  ...
  },
  {
    ...
    header: "IPI Number",
    cell: ({ row }) => {
      const iprsNumber = row.getValue("iprsNumber");
      return (
        <div>
          {iprsNumber ? (
            <div>{String(iprsNumber)}</div>
          ) : (
            <span className="inline-flex items-center rounded-md bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-inset ring-red-600/10">
              None
            </span>
  ...
  },
    {
        ...
        cell: ({ row }) => {
        const artist = row.original;
        return <div> <Link className="bg-blue-600 py-2 px-2 rounded-sm text-white" 
         href={`/artists/${btoa(artist._id as string)}`}>View Profile</Link>
         </div>;
        },
    },
];
